...
const getAllOrders = catchAsyncError(async (req, res, next) => {
  let orders = await orderModel.findOne({}).populate("cartItem.productId");
...
export const getUserOrders = catchAsyncError(async (req, res, next) => {
  const orders = await orderModel.find({ userId: req.user._id }).populate("cartItem.productId");
  res.status(200).json({ message: "success", orders });
});

// Get Orders for Sellers
export const getSellerOrders = catchAsyncError(async (req, res, next) => {
  const orders = await orderModel.find({ "cartItem.sellerId": req.user._id }).populate("cartItem.productId");
  res.status(200).json({ message: "success", orders });
});
